...
意图清晰度分岔（拍板 2026-08-19：人格只在意图模糊时接管落法）：先判断意图的清晰度——
- 意图明确（对象、做法或目标可辨）→ 严格围绕意图生成：每条选项是同一目标的不同手段、路线与分寸；人格不得改变、增删或过滤选项（选项即意图）。
...
function intentDirective(intent) {
  const cleaned = String(intent ?? "")
    ...
    .slice(0, 40);
  if (!cleaned) return "";
  return `\n玩家意图(玩家已声明,本回合选项必须围绕它生成,不得偷换目标):「${cleaned}」。每条选项都必须直接执行或实质推进该意图;若该意图在当前处境无法一步达成,给出为它迈出第一步的具体行动(打听、找门路、做准备),不得替换成无关方向。意图点名的玩家原创实体(context.playerCreations 里列出的门派/身份/地点/物品/人物)按真实世界事实对待。清晰度分岔:该意图明确时严格围绕它生成,人格不改变选项;该意图模糊时(只有情绪/方向、无可执行做法),按玩家心性底色(context.playerCapabilities.bigFive 的档位/行为词/好面坏面)落成「这个性子的人会怎么做」的具体行事——不同选项体现底色不同侧面,但仍回应同一意图,不偷换目标、不设性格门槛。一切硬约束(身份能力/境界/地点/人物可见与存活/已知事实/原著现状)不变。`;
}
