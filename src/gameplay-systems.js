...
// 命数卡的火候档（拍板 2026-08-22：边注「命数」卡）：铺垫期的改命进度
// 以三档意象常驻——不泄露势能数值；临门（≥阈值）亮第三档，与叙事征兆
// （divergenceApproach）同拍；天命反弹势能归零，档位自然跌回。
export function fateTierOf(momentum, threshold) {
  const t = Math.max(1, Number(threshold) || 1);
  const ratio = (Number(momentum) || 0) / t;
  if (ratio >= 1) return 3;
