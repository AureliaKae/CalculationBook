                ...
                {view?.playerSheet ? (
                  <section className="margin-sec" aria-label="此身">
                    ...
                    <button type="button" className="proto-self" onClick={() => setCharOpen(true)}>
                      {/* 无境界体系的书：身份就是「此身」的定位（都市书的职业、
                          历史书的功名、悬疑书的行当——都来自本书身份体系），
                          随身份切换实时更新；不再硬编码「无境界之身」。 */}
                      <span className="proto-self-realm">
                        {view.playerSheet.realm.current ?? view.playerSheet.roleName ?? "平凡之人"}
                      </span>
                      <span className="proto-self-counts">
                        {view.playerSheet.abilityLabel ?? "技能"}
                        {view.playerSheet.abilities.length} · {view.playerSheet.inventoryLabel ?? "行囊"}
                        {view.playerSheet.inventory.length}
                      </span>
                    </button>
                  </section>
                ) : null}
                {/* 命数卡（拍板 2026-08-22）：铺垫期的改命进度常驻边注——目标用
                    称呼、火候用三档意象，不泄露势能数值；临门与叙事征兆同拍。 */}
                {view?.fate?.length ? (
                  <section className="margin-sec" aria-label="命数">
                    ...
                    <ul className="fate-list">
                      {view.fate.map((item) => (
                        <li key={item.label} className={"fate-item tier-" + item.tier}>
                          <span className="fate-label">{item.label}</span>
                          <span className="fate-stage">
                            {["", "暗流初起", "履薄蓄势", "命运松动"][item.tier]}
                          </span>
                        </li>
                      ))}
                    </ul>
