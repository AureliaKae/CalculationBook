...
test("默认地址与凭证标签认注册表厂商", () => {
  ...
  assert.equal(credentialLabel("https://api.moonshot.cn/v1"), "月之暗面 Kimi");
});

test("迁移时静默清除注册表之外的凭证并清空指向它的槽位", () => {
  const migrated = migrateSettings({
    version: 2,
    credentials: [
      { id: "a", baseUrl: "https://api.deepseek.com" },
      { id: "b", baseUrl: "https://api.example.com" },
      { id: "c", baseUrl: "https://dashscope.aliyuncs.com/compatible-mode/v1" },
      { id: "d", baseUrl: "https://api.openai.com" },
    ],
    fast: { credentialId: "b", model: "some-model" },
    strong: { credentialId: "a", model: "deepseek-v4-pro" },
  });
  // 注册表内厂商都保留(含新收录的 OpenAI),未收录的清除。
  assert.deepEqual(migrated.credentials.map((item) => item.id), ["a", "c", "d"]);
...
test("v1 旧格式指向未收录地址时整组丢弃", () => {
  const migrated = migrateSettings({
    baseUrl: "https://api.example.com",
    encryptedKey: "cipher",
    fastModel: "some-model",
    strongModel: "some-model",
  });
  assert.deepEqual(migrated, emptySettings());
});

test("clientConfig 接受阿里千问凭证并拒绝未收录的地址", () => {
  ...
  assert.throws(
    () =>
      clientConfig({
        credentials: [{ id: "a", baseUrl: "https://api.example.com", apiKey: "key" }],
        fast: { credentialId: "a", model: "some-model" },
        strong: { credentialId: "a", model: "some-model" },
      }),
    /仅支持已收录/,
  );
});

test("clientConfig 接受注册表内的新厂商凭证（多密钥拍板 2026-08-22）", () => {
  const kimi = clientConfig({
    credentials: [{ id: "k", baseUrl: "https://api.moonshot.cn/v1", apiKey: "sk-k" }],
    fast: { credentialId: "k", model: "kimi-k2-turbo-preview" },
    strong: { credentialId: "k", model: "kimi-k2-0711-preview" },
  });
  assert.equal(kimi.strong.baseUrl, "https://api.moonshot.cn/v1");
  assert.equal(kimi.strong.model, "kimi-k2-0711-preview");

  const zhipu = clientConfig({
    credentials: [{ id: "z", baseUrl: "https://open.bigmodel.cn/api/paas/v4", apiKey: "sk-z" }],
    fast: { credentialId: "z", model: "glm-4.5-flash" },
    strong: { credentialId: "z", model: "glm-4.6" },
  });
  assert.equal(zhipu.fast.model, "glm-4.5-flash");
});
